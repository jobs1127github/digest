...
(function($) { // hide the namespace
	...
	$
			.extend(
					Datepick.prototype,
					{
						...
						_mainDivId : 'datepick-div', // The ID of the main
						// datepicker division
						_inlineClass : 'datepick-inline', // The name of the
						// inline marker
						// class
						_appendClass : 'datepick-append', // The name of the
						// append marker
						// class
						_triggerClass : 'datepick-trigger', // The name of the
						// trigger marker
						// class
						_dialogClass : 'datepick-dialog', // The name of the
						// dialog marker
						// class
						_promptClass : 'datepick-prompt', // The name of the
						// dialog prompt
						// marker class
						_disableClass : 'datepick-disabled', // The name of
						// the disabled
						// covering
						// marker class
						_oneMonthClass : 'datepick-one-month', // The name of
						// the single
						// month marker
						// class
						_unselectableClass : 'datepick-unselectable', // The
						// name
						// of
						// the
						// unselectable
						...
						// marker
						// class
						_currentClass : 'datepick-current-day', // The name of
						// the current
						// day marker
						// class
						_dayOverClass : 'datepick-days-cell-over', // The name
						// of the
						// day hover
						// marker
						// class
						_weekOverClass : 'datepick-week-over', // The name of
						// the week
						// hover marker
						// class
						_coverClass : 'datepick-cover', // The name of the IE6-
						// iframe marker class
						 ...
						 * use for this date picker instance
						 */
						...
						/*
						 * Create a new instance object. @param target (jQuery)
						 * the target input field or division or span @param
						 * inline (boolean) true if this datepicker appears
						 * inline
						 */
						...
						/*
						 * Attach the date picker to an input field. @param
						 * target (element) the target input field or division
						 * or span @param inst (object) the instance settings
						 * for this datepicker
						 */
						...
						/*
						 * Attach an inline date picker to a div. @param target
						 * (element) the target input field or division or span
						 * @param inst (object) the instance settings for this
						 * datepicker
						 */
						_inlineDatepick : function(target, inst) {
							...
							inst.dpDiv
									.width(this._getNumberOfMonths(inst)[1]
											* $('.' + this._oneMonthClass,
						 ...
						 * for the dialog's position within the screen or
						 * (event) with x/y coordinates or leave empty for
						 * default (screen centre)
						 */
						_dialogDatepick : function(input, dateText, onSelect,
								settings, pos) {
						...
						_enableDatepick : function(target) {
							...
							if (nodeName == 'input') {
								...
								inst.siblings.filter(
										'button.' + this._triggerClass).each(
										function() {
											this.disabled = false;
										}).end().filter(
						...
						_isDisabledDatepick : function(target) {
							return (!target ? false : $.inArray(target,
									this._disabledInputs) > -1);
						},
						 ...
						 * above is an object or to retrieve value)
						 */
						...
						/*
						 * Handle keystrokes. @param event (KeyEvent) the
						 * keystroke details @return (boolean) true to continue,
						 * false to discard
						 */
						_doKeyDown : function(event) {
							...
							if ($.datepick._datepickerShowing)
								...
								case 27:
									$.datepick._hideDatepick(null, $.datepick
											._get(inst, 'duration'));
						 ...
						 * @param event (KeyEvent) the keystroke details @return
						 * (boolean) true to continue, false to discard
						 */
						 ...
						 * @param inst (object) the instance settings for this
						 * datepicker @return (string) the set of characters
						 * allowed by this format
						 */
						_possibleChars : function(inst) {
							var dateFormat = $.datepick
						...
						_showDatepick : function(input) {
							...
							if ($.datepick._isDisabledDatepick(input)
									|| $.datepick._lastInput == input) // already
								// here
								return;
							var inst = $.datepick._getInst(input);
							var beforeShow = $.datepick
							...
							if (!$.datepick._pos) { // position below input
								...
								$.datepick._pos[1] += input.offsetHeight; // add
								// the
								// height
							}
							var isFixed = false;
							...
							// fix width for dynamic number of date pickers
							inst.dpDiv.width($.datepick
									._getNumberOfMonths(inst)[1]
									* $('.' + $.datepick._oneMonthClass,
											inst.dpDiv).width());
							// and adjust position before showing
							offset = $.datepick._checkOffset(inst, offset,
									isFixed);
							inst.dpDiv
									.css({
										position : ($.datepick._inDialog
												&& $.blockUI ? 'static'
												: (isFixed ? 'fixed'
														: 'absolute')),
										display : 'none',
							...
							if (!inst.inline) {
								var showAnim = $.datepick
										._get(inst, 'showAnim')
										|| 'show';
								var duration = $.datepick
										._get(inst, 'duration');
								var postProcess = function() {
									$.datepick._datepickerShowing = true;
									var borders = $.datepick
											._getBorders(inst.dpDiv);
									inst.dpDiv.find(
											'iframe.' + $.datepick._coverClass)
											. // IE6- only
											css(
													{
														left : -borders[0],
								...
								if ($.effects && $.effects[showAnim])
									inst.dpDiv.show(showAnim, $.datepick._get(
											inst, 'showOptions'), duration,
											postProcess);
								else
						...
						/*
						 * Generate the date picker content. @param inst
						 * (object) the instance settings for this datepicker
						 */
						_updateDatepick : function(inst) {
							var borders = this._getBorders(inst.dpDiv);
							inst.dpDiv.empty().append(this._generateHTML(inst))
									.find('iframe.' + this._coverClass). // IE6-
									// only
									css({
										left : -borders[0],
										top : -borders[1],
						 ...
						 * inst (object) the instance settings for this
						 * datepicker @param offset (object) the offset of the
						 * attached field @param isFixed (boolean) true if
						 * control or a parent is 'fixed' in position @return
						...
						_checkOffset : function(inst, offset, isFixed) {
							...
							var scrollY = document.documentElement.scrollTop
									|| document.body.scrollTop;
							// reposition date picker horizontally if outside
							// the browser window
							...
							// reposition date picker vertically if outside the
							// browser window
						...
						/*
						 * Find an element's position on the screen. @param elem
						 * (element) the element to check @return (number[2])
						 * the x- and y-coordinates for the object
						 */
						_findPos : function(elem) {
							while (elem
									&& (elem.type == 'hidden' || elem.nodeType != 1)) {
						 ...
						 * (element) the input field attached to the date picker
						 * @param duration (string) the duration over which to
						...
						_hideDatepick : function(input, duration) {
							...
							if (this._datepickerShowing) {
								...
								if (duration != '' && $.effects
										&& $.effects[showAnim])
									inst.dpDiv.hide(showAnim, $.datepick._get(
											inst, 'showOptions'), duration,
											postProcess);
								else
						...
						/*
						 * Tidy up after a dialog display. @param inst (object)
						...
						_checkExternalClick : function(event) {
							...
							if (!$target.parents().andSelf().is(
									'#' + $.datepick._mainDivId)
									&& !$target
											.hasClass($.datepick.markerClassName)
									&& !$target.parents().andSelf().hasClass(
											$.datepick._triggerClass)
						...
						/*
						 * Adjust one of the date sub-fields. @param id (string)
						 * the ID of the target field @param offset (number) the
						 * amount to change by @param period (string) 'D' for
						 * days, 'M' for months, 'Y' for years
						 */
						...
						/*
						 * Selecting a new month/year. @param id (string) the ID
						 * of the target field @param select (element) the
						 * select being chosen from @param period (string) 'M'
						...
						/*
						 * Action for changing the first week day. @param id
						 * (string) the ID of the target field @param day
						 * (number) the number of the first day, 0 = Sun, 1 =
						 * Mon, ...
						 */
						...
						/*
						 * Hover over a particular day. @param id (string) the
						 * ID of the target field @param year (number) the year
						 * for this day @param month (number) the month for this
						 * day @param td (element) the table cell containing the
						 * selection
						 */
						...
						/*
						 * Select a particular day. @param id (string) the ID of
						 * the target field @param year (number) the year for
						 * this day @param month (number) the month for this day
						 * @param td (element) the table cell containing the
						 * selection
						 */
						_selectDay : function(id, year, month, td) {
							...
							if (rangeSelect) {
								...
								if (inst.stayOpen) {
									$('.datepick td', inst.dpDiv).removeClass(
											this._currentClass);
						...
						/*
						 * Update the input field with the selected date. @param
						 * id (string) the ID of the target field @param dateStr
						 * (string) the chosen date
						 */
						_selectDate : function(id, dateStr) {
							...
							if (inst.inline)
								...
								this._hideDatepick(null, this._get(inst,
										'duration'));
						 ...
						 * main field. @param inst (object) the instance
						...
						/*
						 * Provide status text for a particular date. @param
						 * date (Date) the date to get the status for @param
						 * inst (object) the current datepicker instance @return
						 * (string) the status display text for this date
						 */
						dateStatus : function(date, inst) {
							return $.datepick.formatDate($.datepick._get(inst,
									'dateStatus'), date, $.datepick
									._getFormatConfig(inst));
						},

						/*
						 * Parse a string value into a date object. See
						 * formatDate below for the possible formats. @param
						 ...
						 * single quote @param format (string) the desired
						 * format of the date @param date (Date) the date value
						 * to format @param settings (object) attributes
						 * include: dayNamesShort (string[7]) abbreviated names
						 * of the days from Sunday (optional) dayNames
						 * (string[7]) names of the days from Sunday (optional)
						 * monthNamesShort (string[12]) abbreviated names of the
						 * months (optional) monthNames (string[12]) names of
						 * the months (optional) @return (string) the date in
						 * the above format
						 */
						 ...
						 * inst (object) the instance settings for this
						 * datepicker @param name (string) the name of the
						 * property @return (any) the property's value
						 */
						...
						/*
						 * Retrieve the default date shown on opening. @param
						 * inst (object) the instance settings for this
						 * datepicker @return (Date) the default date
						 */
						_getDefaultDate : function(inst) {
							return this._restrictMinMax(inst, this
									._determineDate(this._get(inst,
											'defaultDate'), new Date()));
						},

						/*
						 * A date may be specified as an exact value or a
						 * relative one. @param date (Date or number or string)
						 * the date or offset @param defaultDate (Date) the date
						 * to use if no other supplied @return (Date) the
						 * decoded date
						 */
						...
						/*
						 * Set the date(s) directly. @param inst (object) the
						 * instance settings for this datepicker @param date
						 * (Date or number or string) the new date or start of a
						 * range @param endDate (Date or number or string) the
						 * end of a range
						 */
						_setDate : function(inst, date, endDate) {
							...
							if (this._get(inst, 'rangeSelect')) {
								if (endDate) {
									endDate = this._restrictMinMax(inst, this
						...
						/*
						 * Retrieve the date(s) directly. @param inst (object)
						 * the instance settings for this datepicker @return
						 * (Date or Date[2]) the current date or dates (for a
						 * range)
						 */
						 ...
						 * picker. @param inst (object) the instance settings
						 * for this datepicker @return (string) the new HTML for
						 * the datepicker
						 */
						_generateHTML : function(inst) {
							...
							var showBigPrevNext = this._get(inst,
									'showBigPrevNext');
							var numMonths = this._getNumberOfMonths(inst);
							...
							if (maxDate) { // don't show past maximum unless
								// also restricted by minimum
								var maxDraw = this
							...
							var prevBigText = (showBigPrevNext ? this._get(
									inst, 'prevBigText') : '');
							...
							var prev = '<div class="datepick-prev">'
									+ (this._canAdjustMonth(inst, -1, drawYear,
											drawMonth) ? (showBigPrevNext ? '<a href="javascript:void(0)" onclick="jQuery.datepick._adjustDate(\'#'
											+ inst.id
											+ '\', -'
											+ stepBigMonths
											+ ', \'M\');"'
									...
									: this.formatDate(nextText, this
											._daylightSavingAdjust(new Date(
													drawYear, drawMonth
															+ stepMonths, 1)),
											this._getFormatConfig(inst)));
							var nextBigText = (showBigPrevNext ? this._get(
									inst, 'nextBigText') : '');
							...
							var next = '<div class="datepick-next">'
									+ (this._canAdjustMonth(inst, +1, drawYear,
											drawMonth) ? '<a href="javascript:void(0)" onclick="jQuery.datepick._adjustDate(\'#'
											+ inst.id
											+ '\', +'
											+ stepMonths
											+ ', \'M\');"'
							...
							html += (showStatus ? '<div style="clear: both;"></div><div id="datepick-status-'
									+ inst.id
									...
									+ ($.browser.msie
											...
											&& !inst.inline ? '<iframe src="javascript:false;" class="'
						...
						/*
						 * Generate the month and year header. @param inst
						 * (object) the instance settings for this datepicker
						 * @param drawMonth (number) the current month @param
						 * drawYear (number) the current year @param minDate
						 * (Date) the minimum allowed date or null if none
						 * @param maxDate (Date) the maximum allowed date or
						 * null if none @return (string) the HTML for the month
						 * and year
						 */
						_generateMonthYearHeader : function(inst, drawMonth,
								drawYear, minDate, maxDate, selectedDate,
								secondary, showStatus, initStatus, monthNames) {
							...
							if (secondary || !changeMonth)
								monthHtml += '<span>' + monthNames[drawMonth]
										+ '</span>';
							else {
						 ...
						 * @param showStatus (boolean) true if the status bar is
						 * shown @param id (string) the ID of the datepicker
						 * instance @param text (string) the status text to
						 * display @param initStatus (string) the default status
						 * message @return (string) hover actions for the status
						 * messages
						 */
						...
						/*
						 * Adjust one of the date sub-fields. @param inst
						 * (object) the instance settings for this datepicker
						 * @param offset (number) the change to apply @param
						 * period (string) 'D' for days, 'M' for months, 'Y' for
						 * years
						 */
						...
						/*
						 * Ensure a date is within any min/max bounds. @param
						 * inst (object) the instance settings for this
						 * datepicker @param date (Date) the date to check
						...
						/*
						 * Notify change of month/year. @param inst (object) the
						...
						/*
						 * Determine the number of months to show. @param inst
						 * (object) the instance settings for this datepicker
						 * @return (number[2]) the number of rows and columns to
						 * display
						 */
						...
						/*
						 * Determine the current minimum/maximum date. Ensure no
						 * time components are set. May be overridden for a
						 * range. @param inst (object) the instance settings for
						 * this datepicker @param minMax (string) 'min' or 'max'
						 * for required date @param checkRange (boolean) true to
						 * allow override for a range minimum @return (Date) the
						 * minimum/maximum date or null if none
						 */
						 ...
						 * month (0 to 11) @return (number) the number of days
						 * in this month
						 */
						 ...
						 * @param year (number) the full year @param month
						 * (number) the month (0 to 11) @return (number) 0 =
						 * Sunday, 1 = Monday, ...
						 */
						...
						/*
						 * Determines if we should allow a "prev/next" month
						 * display change. @param inst (object) the instance
						 * settings for this datepicker @param offset (number)
						 * the number of months to change by @param curYear
						 * (number) the full current year @param curMonth
						 * (number) the current month (0 to 11) @return
						 * (boolean) true if prev/next allowed, false if not
						 */
						...
						/*
						 * Is the given date in the accepted range? @param inst
						 * (object) the instance settings for this datepicker
						 * @param date (Date) the date to check @return
						 * (boolean) true if the date is in the allowed
						 ...
						 * formatting/parsing. @param inst (object) the instance
						 * settings for this datepicker @return (object) the
						 * settings subset
						 */
						_getFormatConfig : function(inst) {
							return {
								shortYearCutoff : this._get(inst,
										'shortYearCutoff'),
								dayNamesShort : this
										._get(inst, 'dayNamesShort'),
						...
						/*
						 * Format the given date for display. @param inst
						 * (object) the instance settings for this datepicker
						 * @param day (number, optional) the day of the month
						 * @param month (number, optional) the month of the year
						 * (0 to 11) @param year (number, optional) the full
						 * year @return (string) formatted date
						 */
	...
	$.fn.datepick = function(options) {
		...
		return this.each(function() {
			typeof options == 'string' ? $.datepick['_' + options + 'Datepick']
					.apply($.datepick, [ this ].concat(otherArgs)) : $.datepick
	...
	$(function() {
		$(document).mousedown($.datepick._checkExternalClick).find('body')
